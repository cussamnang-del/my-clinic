...
class DocumentController extends Controller
{
  ...
  public function filterTreatment(Request $request)
  {
    if($request->ajax()){
      if($request->from_date != '' && $request->to_date != ''){
        $data['hts'] = HospitalTreatment::whereBetween('ht_date', array($request->from_date, $request->to_date))
                                          ->get();
        $data['hnotes'] = HNote::whereBetween('date', array($request->from_date, $request->to_date))
                                ->get();
      } else {
