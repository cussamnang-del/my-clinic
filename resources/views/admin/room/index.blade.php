...
@section('content')
    ...
    <div class="card-body">
      <div class="table-responsive">
        <table id="datatable" class="table table-striped table-bordered">
          <thead>
            <tr>
              <th>{{ trans('cruds.room.fields.id') }}</th>
              <th>{{ trans('cruds.room.fields.room_no') }}</th>
              <th>{{ trans('cruds.room.fields.created_at') }}</th>
  ...
  @include('admin.room.templates.crudModal')
@endsection
  ...
  <script>
    $(document).ready(function () {
      ...
      $('#addNewObject').on('click',function(e){
        e.preventDefault();
        $('#crudObjectModal').find('.modal-title').html('{{ trans('global.add') }} {{ trans('cruds.room.title_singular') }}');
      ...
      $('body').on('click', 'a#objectEdit', function (e) {
        ...
        modal.find('.modal-title').html('{{ trans('global.edit') }} {{ trans('cruds.room.title_singular') }}');
        $.get( actionUrl +'/' +object_id+'/edit', function (res) {
          form.find('#object_id').val(res.data.id);
          form.find('#room_no').val(res.data.room_no);
      ...
      $('body').on('change','.ace-switch',function(e){
        ...
        $.ajax({
          type : 'GET',
          dataType: 'JSON',
          url :'{{ route('admin.rooms.changeStatus') }}',
          data: {
            'status':status,
            'object_id':object_id
          },
