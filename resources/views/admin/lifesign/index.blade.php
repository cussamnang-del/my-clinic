...
@section('content')
  @section('breadcrumb',trans('cruds.lifesign.title'))
  <div class="card">
    <div class="card-header">
      <h4 class="mb-0 text-primary"><i class="bx bxs-user me-1 font-22 text-primary"></i>{{ trans('global.list') }} {{ trans('cruds.lifesign.title') }}
        @can($prefix.'create')
    ...
    <div class="card-body">
      <div class="table-responsive">
        <table id="datatable" class="table table-striped table-bordered">
          <thead>
            <tr>
              <th>{{ trans('cruds.lifesign.fields.id') }}</th>
              <th>{{ trans('cruds.lifesign.fields.type') }}</th>
              <th>{{ trans('cruds.lifesign.fields.name') }}</th>
              <th>{{ trans('cruds.lifesign.fields.created_at') }}</th>
              <th>{{ trans('global.status') }}</th>
              <th>{{ trans('global.action') }}</th>
            </tr>
          </thead>
          <tbody id="objectList">
            @foreach ($all_lifesigns as $row)
  ...
  @include('admin.lifesign.templates.crudModal')
@endsection
  ...
  <script>
    $(document).ready(function () {
      ...
      $('#addNewObject').on('click',function(e){
        e.preventDefault();
        $('#crudObjectModal').find('.modal-title').html('{{ trans('global.add') }} {{ trans('cruds.lifesign.title_singular') }}');
      ...
      $('body').on('click', 'a#objectEdit', function (e) {
        ...
        modal.find('.modal-title').html('{{ trans('global.edit') }} {{ trans('cruds.lifesign.title_singular') }}');
        $.get( actionUrl +'/' +object_id+'/edit', function (res) {
          form.find('#object_id').val(res.data.id);
          if ($('#frmCrudObject #type_id').find("option[value='" + res.data.type_id + "']").length) {
      ...
      $('body').on('change','.ace-switch',function(e){
        ...
        $.ajax({
          type : 'GET',
          dataType: 'JSON',
          url :'{{ route('admin.lifesigns.changeStatus') }}',
          data: {
            'status':status,
            'object_id':object_id
          },
