...
@section('content')
@section('breadcrumb',trans('cruds.product.title'))
  <div class="card">
    <div class="card-header">
      <h4 class="mb-0 text-primary"><i class="bx bxs-user me-1 font-22 text-primary"></i>{{ trans('global.list') }} {{ trans('cruds.product.title') }}
        @can($prefix.'create')
        ...
        <table id="datatable" class="table table-striped table-bordered">
          <thead>
            <tr>
              ...
              <th>{{ trans('cruds.product.fields.p_name') }}</th>
              <th>{{ trans('cruds.product.fields.p_code') }}</th>
              <th>{{ trans('cruds.product.fields.country') }}</th>
              <th>{{ trans('cruds.product.fields.description') }}</th>
              <th>{{ trans('cruds.product.fields.unit') }}</th>
              <th>{{ trans('cruds.product.fields.strength') }}</th>
          ...
          <tfoot>
            <tr>
              ...
              <th>{{ trans('cruds.product.fields.p_name') }}</th>
              <th>{{ trans('cruds.product.fields.p_code') }}</th>
              <th>{{ trans('cruds.product.fields.group_id') }}</th>
              <th>{{ trans('cruds.product.fields.type_id') }}</th>
              <th>{{ trans('cruds.product.fields.country') }}</th>
              <th>{{ trans('cruds.product.fields.description') }}</th>
              <th>{{ trans('cruds.product.fields.image') }}</th>
              <th>{{ trans('global.status') }}</th>
  ...
  @include('admin.product.templates.crudModal')
@endsection
  ...
  <script>
    $(document).ready(function () {
      ...
      $('#addNewObject').on('click',function(e){
        e.preventDefault();
        $('#crudObjectModal').find('.modal-title').html('{{ trans('global.add') }} {{ trans('cruds.product.title_singular') }}');
      ...
      $('body').on('click', 'a#objectEdit', function (e) {
        ...
        $.get( actionUrl +'/' +object_id+'/edit', function (res) {
          ...
          form.find('#p_name').val(res.data.p_name);
          form.find('#p_code').val(res.data.p_code);
          form.find('#unit').val(res.data.unit);
          form.find('#strength').val(res.data.strength);
          form.find('#group_id').val(res.data.group_id);
          form.find('#type_id').val(res.data.type_id);
          form.find('#country').val(res.data.country);
