...
@section('content')
  ...
  <div class="card">
    <form id="frmCrudObject" action="{{ route('admin.'.$crudRoutePath.'.update',$schedule->id) }}" method="post">
      {{ csrf_field() }}
      @method('PUT')
        ...
        <div class="row mb-2">
          <div class="col-lg-9 col-md-9 col-sm-9">
            <div class="form-group">
              ...
              <input id="title" value="{{$schedule->title}}" name="title" class="form-control @error('title') is-invalid @enderror" type="text" placeholder="{{ trans('cruds.schedule.fields.title') }}">
              @error('title')
          ...
          <div class="col-lg-3 col-md-3 col-sm-3">
            <div class="form-group">
              ...
              <input id="color" value="{{$schedule->color}}" name="color" type="text" class="form-control color @error('title') is-invalid @enderror" aria-label="Text input with checkbox">
              @error('color')
                <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
                </span>
        ...
        <div class="row mb-2">
          <div class="col-lg-6 col-md-6 col-sm-6">
            <div class="form-group">
              ...
              <input id="start_time" value="{{$schedule->start_time}}" name="start_time" class="form-control datetime @error('start_time') is-invalid @enderror" type="text" placeholder="{{ trans('cruds.schedule.fields.start_time') }}">
              @error('start_time')
                <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
                </span>
          ...
          <div class="col-lg-6 col-md-6 col-sm-6">
            <div class="form-group">
              ...
              <input id="finish_time" value="{{$schedule->finish_time}}" name="finish_time" class="form-control datetime @error('finish_time') is-invalid @enderror" type="text" placeholder="{{ trans('cruds.schedule.fields.finish_time') }}">
              @error('finish_time')
                <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="form-group">
              ...
              <select id="customer_id" name="customer_id" class="form-control single-select @error('customer_id') is-invalid @enderror"  style="width: 100%" data-placeholder="{{ trans('global.select') }} {{ trans('cruds.schedule.fields.customer_id') }}">
                ...
                @foreach ($customers as $row)
                  <option value="{{ $row->id}}" {{$schedule->customer_id==$row->id ? 'selected':''}}>{{ $row->name }}</option>
                @endforeach
              </select>
              @error('customer_id')
                <span class="invalid-feedback" role="alert">
            ...
            <div class="form-group">
              ...
              <select id="user_id" name="user_id" class="form-control single-select @error('user_id') is-invalid @enderror"  style="width: 100%" data-placeholder="{{ trans('global.select') }} {{ trans('cruds.schedule.fields.user_id') }}">
                ...
                @foreach ($users as $row)
                  <option value="{{ $row->id}}" {{$schedule->user_id==$row->id ? 'selected':''}}>{{ $row->name }}</option>
                @endforeach
              </select>
              @error('user_id')
                <span class="invalid-feedback" role="alert">
        ...
        <div class="row mb-2">
          <div class="col-lg-12 col-md-12 col-sm-12">
            <div class="form-group">
              ...
              <input id="desr" value="{{$schedule->desr}}" name="desr" class="form-control" type="text" placeholder="{{ trans('cruds.schedule.fields.desr') }}">
            </div>
          </div>
        </div>
        <div class="row mb-2">
          <div class="col-lg-6 col-md-6 col-sm-6">
            <div class="form-group">
              ...
              <select id="status" value="{{$schedule->title}}" name="status" class="form-control single-select"  style="width: 100%" data-placeholder="{{ trans('global.select') }} {{ trans('cruds.schedule.fields.status') }}">
                @foreach (App\Models\Schedule::APPOINTMENT_STATUS_RADIO as $key => $label)
                  <option value="{{ $key}}" {{$schedule->status==$key ? 'selected':''}}>{{ $label }}</option>
                @endforeach
              </select>
            </div>
          </div>
      ...
      <div class="card-footer text-center">
        ...
        <button type="submit" class="btn btn-sm btn-outline-success px-3 radius-30">
