...
@section('content')
  @section('breadcrumb',trans('cruds.company_information.title'))
  <div class="card">
    <div class="card-header">
      <h4 class="mb-0 text-primary"><i class="bx bxs-user me-1 font-22 text-primary"></i>{{ trans('global.list') }} {{ trans('cruds.company_information.title') }}
        @can($prefix.'create')
    ...
    <div class="card-body">
      <div class="table-responsive">
        <table id="datatable" class="table table-striped table-bordered">
          <thead>
            <tr>
              <th>{{ trans('cruds.company_information.fields.id') }}</th>
              <th>{{ trans('cruds.company_information.fields.name_en') }}</th>
              <th>{{ trans('cruds.company_information.fields.name_kh') }}</th>
              <th>{{ trans('cruds.company_information.fields.address') }}</th>
              <th>{{ trans('cruds.company_information.fields.phone1') }}</th>
              <th>{{ trans('cruds.company_information.fields.phone2') }}</th>
              <th>{{ trans('cruds.company_information.fields.phone3') }}</th>
              <th>{{ trans('cruds.company_information.fields.created_at') }}</th>
              <th>{{ trans('global.status') }}</th>
              <th>{{ trans('global.action') }}</th>
            </tr>
          </thead>
          <tbody id="objectList">
            @foreach ($company_informations as $row)
  ...
  @include('admin.company_information.templates.crudModal')
@endsection
...
@push('scripts')
  ...
  <script>
    $(document).ready(function () {
      ...
      $('#addNewObject').on('click',function(e){
        e.preventDefault();
        $('#crudObjectModal').find('.modal-title').html('{{ trans('global.add') }} {{ trans('cruds.company_information.title_singular') }}');
      ...
      $('body').on('click', 'a#objectEdit', function (e) {
        ...
        modal.find('.modal-title').html('{{ trans('global.edit') }} {{ trans('cruds.company_information.title_singular') }}');
        $.get( actionUrl +'/' +object_id+'/edit', function (res) {
          form.find('#object_id').val(res.data.id);
          form.find('#name_en').val(res.data.name_en);
          form.find('#name_kh').val(res.data.name_kh);
          form.find('#address').val(res.data.address);
          form.find('#phone1').val(res.data.phone1);
          form.find('#phone2').val(res.data.phone2);
          form.find('#phone3').val(res.data.phone3);
          form.find('#old_image').val(res.data.logo);
          form.find('#preview').attr('src',"{{ asset('uploads/logo/') }}"+'/'+res.data.logo);
      ...
      $('body').on('change','.ace-switch',function(e){
        ...
        $.ajax({
          type : 'GET',
          dataType: 'JSON',
          url :'{{ route('admin.company_informations.changeStatus') }}',
          data: {
            'status':status,
            'object_id':object_id
          },
  ...
  <script>
    ...
    function removeImage() {
      $('#preview').attr('src',"{{ asset('images/image-icon.png') }}");
      $("#remove").val(1);
    }
  </script>
@endpush
