...
@push('styles')
  ...
  <link rel="stylesheet" href="{{ assetUrl() }}/plugins/bootstrap-datetime/css/bootstrap-datetimepicker.css" crossorigin="anonymous"/>
        ...
        <div class="row justify-content-evenly  ">
          <div class="col-lg-9">
            <div class="d-flex align-items-center justify-content-between fs-6 mb-3">
              ...
              <a id="objectOrder" href="#" class="btn btn-info btn-sm text-white" style="cursor: pointer;" data-bs-toggle="tooltip" data-bs-placement="bottom" title="" data-bs-original-title="Show Order Form" aria-label="Edit">Order</a>
...
@push('scripts')
  ...
  <script>
    ...
    $(function () {
      ...
      $('#room_no').select2({
        dropdownParent: $('#addHospitalObjectModal'),
        theme: 'bootstrap4',
         width: $(this).data('width') ? $(this).data('width') : $(this).hasClass('w-100') ? '100%' : 'style',
        placeholder: $(this).data('placeholder'),
        allowClear: Boolean($(this).data('allow-clear')),
  ...
  <script>
    $(document).ready(function () {
      ...
      // $('body').on('click','a#showAnalystForm',function(e){
      //   e.preventDefault();
      //   var pbio_id = $(this).data('id');
      //   var customer_id = $(this).data('customerid');
      //   $.ajax({
      ...
      $('body').on('click','a#objectH',function(e){
        e.preventDefault();
        var modal = $('#addHospitalObjectModal');
        modal.find('#h_name').val($('#rx_bio_name').val());
        modal.find('#h_sex').val($('#rx_bio_sex').val());
        modal.find('#h_age').val($('#rx_bio_age').val());
        modal.find('#h_customer_id').val($('#rx_bio_customer_id').val());
        modal.find('#h_document_id').val($('#rx_bio_document_id').val());
        modal.find('#h_phone_no').val($('#rx_bio_phone_no').val());
        modal.find('#h_address').val($('#rx_bio_address').val());
        var rx_customerId = $('#rx_bio_customer_id').val();
        $.ajax({
          type : 'get',
          dataType: 'JSON',
      ...
      $('#frmAddHospital').on('submit',function(e){
        e.preventDefault();
        var actionUrl = $(this).attr('action');
        var method = $(this).attr('method')
        var modal = $('#addHospitalObjectModal');
        ...
        $.ajax({
          ...
          success: function (res) {
            ...
            if(res.status==400){
              ...
              modal.find('#h_hospital_id').val(res.data.id);
              $('#frmAddHospital').find('#h_note').val('');
              $('#frmAddHospital').find('#room_no').val('').trigger('change');
              modal.modal('hide');
              toastr.success(res.success);
            }
          },
      ...
      $('body').on('click','a#objectHT',function(e){
        e.preventDefault();
        var modal = $('#newHospitalTreatmentObjectModal');
        modal.find('#ht_name').val($('#rx_bio_name').val());
        modal.find('#ht_sex').val($('#rx_bio_sex').val());
        modal.find('#ht_age').val($('#rx_bio_age').val());
        modal.find('#ht_customer_id').val($('#rx_bio_customer_id').val());
        modal.find('#ht_document_id').val($('#rx_bio_document_id').val());
        modal.find('#ht_phone_no').val($('#rx_bio_phone_no').val());
        modal.find('#ht_address').val($('#rx_bio_address').val());
        var rx_customerId = $('#rx_bio_customer_id').val();
        var rx_documentId = $('#rx_bio_document_id').val();
        $.ajax({
          type : 'get',
          ...
          success: function (res) {
            modal.find('#typeA').empty().append(res.typeA);
            modal.find('#typeB').empty().append(res.typeB);
            modal.find('#objectHospitalTreatment').empty().append(res.hts);
          },
          error: function (error) {
            console.log('Error:', error);
            $('#btnObjectSave').html(`{{ trans('global.save')}}`);
                    ...
                    <div class="mb-2" id="${i == 0 ? 'no-delete':null}">
                      <div class="row">
                        ...
                        <div class="col-lg-2 mt-4">
                          <label for="">&nbsp;</label>
                          ${
                            i == 0 ? ` <button type="button" class="btn btn-sm btn-success mt-1 add_more_treatment"><i class="fadeIn animated bx bx-plus-medical"></i>Add</button>`
              ...
              var htmlData = `
                <div class="mb-2" id="no-delete">
                  <div class="row">
                    ...
                    <div class="col-lg-2 mt-4">
                      <label for="">&nbsp;</label>
                      <button type="button" class="btn btn-sm btn-success mt-1 add_more_medicine"><i class="fadeIn animated bx bx-plus-medical"></i> Add</button>
                    </div>
                  </div>
                </div>
              `;
        ...
        $.ajax({
          ...
          success: function (res) {
            if(res.status==400){
              ...
              modal.modal('hide');
              // $('#printReceiptModal').find('#print-content').empty().append(res.receipt);
      ...
      //     type : 'get',
      //     dataType: 'JSON',
      //     url: '{{ route("admin.documents.getItemType") }}',
      //     data: {
        ...
        <div class="mb-2">
          <div class="row">
            ...
            <div class="col-lg-2 mt-1">
              <label for="">&nbsp;</label>
              <button type="button" class="btn btn-sm btn-danger mt-1 remove_button_treatment"><i class="fadeIn animated bx bx-trash"></i> Remove</button>
            </div>
          </div>
        </div>
      `);
  ...
  <script>
    ...
    var $fieldMedicine = $(`
      <tr>
        <td>
          <div class="form-group">
            <select data-id="${i}" id="order_product" name="order_product[]" class="form-control order_product order_detail"  style="width: 100%" data-placeholder="Select Medecine">
        ...
        <td>
          <div class="form-group">
            <input data-id="${i}" id="unit" name="unit[]" class="form-control order_detail" type="text" placeholder="{{ trans('cruds.orderDetail.fields.unit') }}">
          </div>
        </td>
        <td>
          <div class="form-group">
            <input data-id="${i}" id="strength" name="strength[]" class="form-control order_detail" type="text" placeholder="{{ trans('cruds.orderDetail.fields.strength') }}">
          </div>
        </td>
        <td>
          <div class="form-group">
            <input data-id="${i}" id="qty" name="qty[]" class="form-control order_detail" type="text" placeholder="{{ trans('cruds.orderDetail.fields.qty') }}">
          </div>
        </td>
        <td>
          <div class="form-group">
            <input data-id="${i}" id="price" name="price[]" class="form-control order_detail" type="text" placeholder="{{ trans('cruds.orderDetail.fields.price') }}">
          </div>
        </td>
        <td>
          <div class="form-group">
            <input data-id="${i}" id="total" name="total[]" class="form-control order_detail" type="text" placeholder="{{ trans('cruds.orderDetail.fields.total') }}">
          </div>
        </td>
        <td>
          <div class="form-group">
            <input data-id="${i}" id="how_to_use" name="how_to_use[]" class="form-control order_detail" type="text" placeholder="{{ trans('cruds.orderDetail.fields.how_to_use') }}">
          </div>
        </td>
        <td>
          <div class="form-group">
  ...
  <script>
    ...
    var $fieldInjection = $(`
      <tr>
        <td>
          <div class="form-group">
            <select data-id="${j}" id="injection_product" name="injection_product[]" class="form-control order_product order_detail"  style="width: 100%" data-placeholder="Select Medecine">
              <option value="">{{ trans('global.select') }} {{ trans('cruds.order.fields.medicine') }}</option>
              @foreach ($products as $key => $row)
                <option value="{{ $row->id }}">{{ $row->p_name }}</option>
              @endforeach
        ...
        <td>
          <div class="form-group">
            <input data-id="${j}" id="injection_unit" name="injection_unit[]" class="form-control order_detail" type="text" placeholder="{{ trans('cruds.orderDetail.fields.unit') }}">
          </div>
        </td>
        <td>
          <div class="form-group">
            <input data-id="${j}" id="injection_strength" name="injection_strength[]" class="form-control order_detail" type="text" placeholder="{{ trans('cruds.orderDetail.fields.strength') }}">
          </div>
        </td>
        <td>
          <div class="form-group">
            <input data-id="${j}" id="injection_qty" name="injection_qty[]" class="form-control order_detail" type="text" placeholder="{{ trans('cruds.orderDetail.fields.qty') }}">
          </div>
        </td>
        <td>
          <div class="form-group">
            <input data-id="${j}" id="injection_price" name="injection_price[]" class="form-control order_detail" type="text" placeholder="{{ trans('cruds.orderDetail.fields.price') }}">
          </div>
        </td>
        <td>
          <div class="form-group">
            <input data-id="${j}" id="injection_total" name="injection_total[]" class="form-control order_detail" type="text" placeholder="{{ trans('cruds.orderDetail.fields.total') }}">
          </div>
        </td>
        <td>
          <div class="form-group">
            <input data-id="${j}" id="injection_how_to_use" name="injection_how_to_use[]" class="form-control order_detail" type="text" placeholder="{{ trans('cruds.orderDetail.fields.how_to_use') }}">
          </div>
        </td>
        <td>
          <div class="form-group">
