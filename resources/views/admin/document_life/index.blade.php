...
@section('content')
  @section('breadcrumb',trans('cruds.document_life.title'))
  <div class="card">
    <div class="card-header">
      <h4 class="mb-0 text-primary"><i class="bx bxs-user me-1 font-22 text-primary"></i>
        {{ trans('global.list') }} {{ trans('cruds.document_life.title') }}
        @can($prefix.'create')
    ...
    <div class="card-body">
      <div class="table-responsive">
        <table id="datatable" class="table table-striped table-bordered">
          <thead>
            <tr>
              <th>{{ trans('cruds.document_life.fields.id') }}</th>
              <th>{{ trans('cruds.document_life.fields.customer_id') }}</th>
              <th>{{ trans('cruds.document_life.fields.document_id') }}</th>
              <th>{{ trans('cruds.document_life.fields.coltype') }}</th>
              <th>{{ trans('cruds.document_life.fields.colfield') }}</th>
              <th>{{ trans('cruds.document_life.fields.coldesr') }}</th>
              <th>{{ trans('cruds.document_life.fields.num') }}</th>
              <th>{{ trans('cruds.document_life.fields.created_at') }}</th>
              <th>{{ trans('global.status') }}</th>
              <th>{{ trans('global.action') }}</th>
            </tr>
          </thead>
          <tbody id="objectList">
            @foreach ($documentlives as $row)
              <tr id="tr_object_id_{{ $row->id }}">
                <td>{{ $row->id }}</td>
                <td>{{ $row->customer->name }}</td>
                <td>{{ $row->document_id }}</td>
                <td>{{ $row->colType->name }}</td>
  ...
  @include('admin.document_life.templates.crudModal')
@endsection
  ...
  <script>
    $(document).ready(function () {
      ...
      $('#addNewObject').on('click',function(e){
        e.preventDefault();
        $('#crudObjectModal').find('.modal-title').html('{{ trans('global.add') }} {{ trans('cruds.document_life.title_singular') }}');
      ...
      $('body').on('click', 'a#objectEdit', function (e) {
        ...
        modal.find('.modal-title').html('{{ trans('global.edit') }} {{ trans('cruds.document_life.title_singular') }}');
        $.get( actionUrl +'/' +object_id+'/edit', function (res) {
          form.find('#object_id').val(res.data.id);
          if ($('#frmCrudObject #document_id').find("option[value='" + res.data.document_id + "']").length) {
            $('#document_id').val(res.data.document_id).trigger('change');
          }
          form.find('#coltype').val(res.data.coltype);
          form.find('#colfield').val(res.data.colfield);
          form.find('#coldesr').val(res.data.coldesr);
          form.find('#num').val(res.data.num);
      ...
      $('body').on('change','.ace-switch',function(e){
        ...
        $.ajax({
          type : 'GET',
          dataType: 'JSON',
          url :'{{ route('admin.document_lives.changeStatus') }}',
          data: {
            'status':status,
            'object_id':object_id
          },
